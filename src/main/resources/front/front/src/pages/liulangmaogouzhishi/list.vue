...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"40px 0 0","alignItems":"center","flexWrap":"wrap","textAlign":"center","background":"#f5f5f5","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>发布标题</div>
        <el-input v-model="formSearch.zhishibiaoti" placeholder="发布标题" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>发布类型</div>
        <el-select v-model="formSearch.zhishileixing" placeholder="请选择发布类型" :clearable="true">
          <el-option v-for="(item, index) in zhishileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>发布人</div>
        <el-input v-model="formSearch.xingtaitezheng" placeholder="发布人" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('liulangmaogouzhishi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"0","background":"#9f9fd7","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/liulangmaogouzhishiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"padding":"20px 7% 20px 7%","margin":"20px 0 10px","background":"none","flex":"1","width":"100%","minWidth":"900px","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 3%","margin":"30px 0 0","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
			<div :style='{"cursor":"pointer","margin":"0 3% 10% 0","flexWrap":"wrap","background":"none","display":"flex","width":"30%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"padding":"20px 10px","margin":"0 0 12px","objectFit":"contain","borderRadius":"8px","background":"url(http://codegen.caihongy.cn/20230129/b183ae010bb2489e8747e9baffeed40f.png) no-repeat center top / 100% 100%","display":"block","width":"100%","height":"260px"}' v-if="item.chongwutupian && item.chongwutupian.substr(0,4)=='http'" :src="item.chongwutupian" class="image" />
				<img :style='{"padding":"20px 10px","margin":"0 0 12px","objectFit":"contain","borderRadius":"8px","background":"url(http://codegen.caihongy.cn/20230129/b183ae010bb2489e8747e9baffeed40f.png) no-repeat center top / 100% 100%","display":"block","width":"100%","height":"260px"}' v-else :src="baseUrl + (item.chongwutupian?item.chongwutupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"4px 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"#efeffd","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.zhishibiaoti}}</div>
				<div :style='{"padding":"4px 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"#efeffd","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.zhishileixing}}</div>
				<div :style='{"padding":"4px 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"#efeffd","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.xingtaitezheng}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/zhishileixing/zhishileixing').then(res => {
        if (res.data.code == 0) {
          this.zhishileixingOptions = res.data.data;
        }
      });
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.zhishibiaoti != '') searchWhere.zhishibiaoti = '%' + this.formSearch.zhishibiaoti + '%';
        if (this.formSearch.zhishileixing != '') searchWhere.zhishileixing = this.formSearch.zhishileixing;
        if (this.formSearch.xingtaitezheng != '') searchWhere.xingtaitezheng = '%' + this.formSearch.xingtaitezheng + '%';
        if (this.curFenlei != '全部') searchWhere.zhishileixing = this.curFenlei;
        this.$http.get('liulangmaogouzhishi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/liulangmaogouzhishiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
