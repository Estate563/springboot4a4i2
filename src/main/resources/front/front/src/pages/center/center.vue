<template>
<div class="center-preview" :style='{"width":"100%","margin":"0 auto","position":"relative","background":"url( ) no-repeat center bottom / 100% auto"}'>
    ...
    <el-tabs tab-position="left" :style='{"background":"none"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="150px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='yonghu'" label="用户名" prop="xingming">
            <el-input v-model="sessionForm.xingming" placeholder="用户名" ></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('yonghu' == this.userTableName) {
        this.$set(this.rules, 'xingming', [{ required: true, message: '请输入用户名', trigger: 'blur' }]);
      }
			if ('yonghu' == this.userTableName) {
