	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20230303/76c3e36e1fa546ef8e134d65e6665618.jpg) center bottom/cover no-repeat fixed","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="用户名" prop="xingming">
					<el-input v-model="ruleForm.xingming" placeholder="用户名" clearable  :readonly="ro.xingming"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="用户名" prop="xingming">
					<el-input v-model="ruleForm.xingming" placeholder="用户名" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="空闲时间" prop="kongxianshijian">
					<el-input v-model="ruleForm.kongxianshijian" placeholder="空闲时间" clearable  :readonly="ro.kongxianshijian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="空闲时间" prop="kongxianshijian">
					<el-input v-model="ruleForm.kongxianshijian" placeholder="空闲时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="申请原因" prop="shenqingyuanyin">
					<el-input v-model="ruleForm.shenqingyuanyin" placeholder="申请原因" clearable  :readonly="ro.shenqingyuanyin"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="申请原因" prop="shenqingyuanyin">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='kongxianshijian'){
							this.ruleForm.kongxianshijian = obj[o];
							this.ro.kongxianshijian = true;
							continue;
						}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shenqingneirong = this.ruleForm.shenqingneirong.replace(reg,'../../../springboot4a4i2/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhiyuanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhiyuanshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
